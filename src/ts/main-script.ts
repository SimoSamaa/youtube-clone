...
type num = number;
type bool = boolean;
type dataTypes = (str | num | bool);
type event = (MouseEvent | any);

// select element
const startContainer = document.querySelector(".start") as HTMLElement;
...
const inputSearch = document.querySelector("input") as HTMLInputElement;
const btnSearch = document.querySelector(".search-btn") as HTMLButtonElement;
const btnRemoveValue_search = document.querySelector(".remove-value_search") as HTMLButtonElement;
const btnAddInput_search = document.querySelector(".btn-add_input-search") as HTMLButtonElement;
const btnBackInput_search = document.querySelector(".btn-back_input-search") as HTMLButtonElement;
const videoPlayer = document.querySelector(".video-player") as HTMLElement;
const popUp = document.querySelector(".pop-up") as HTMLElement;
const video = document.querySelector("video") as HTMLVideoElement;
const btnPlayPause = document.querySelector(".btn-play-pause") as HTMLButtonElement;
const btnTheater = document.querySelector(".btn-theater") as HTMLButtonElement;
const btnfullScreen = document.querySelector(".btn-full-screen") as HTMLButtonElement;
const btnMiniPlayer = document.querySelector(".btn-mini-player") as HTMLButtonElement;
const btnVolumeLvl = document.querySelector(".btn-volume-level") as HTMLButtonElement;
const lineProgressRange = document.querySelector(".line-progress") as HTMLElement;
const volumeRange = document.querySelector("[type='range']") as HTMLInputElement;

// inherit element
const htmlEelementInherit: HTMLElement | str =
    `<!-- menu -->
<button class="menu">
...
;[ ...navHeader.children ].forEach((el, index) => {
    if (index == 0) el.setAttribute("tabindex", "0");
    if (index == 1) el.setAttribute("tabindex", "0");
});

// add class active nav
startContainer.children[ 0 ].addEventListener("click", (): void => {
    nav.classList.add("active-nav");

    // remove class active nav
    ...
    navHeader.children[ 0 ].addEventListener("click", (): void => {
        nav.classList.remove("active-nav");
        document.body.style.overflowY = "scroll";
        videoPlayer.style.zIndex = "1";
    });
...
window.addEventListener('keyup', (e: KeyboardEvent): void => {
    if (e.key.includes("Enter")) return buttonSearch();
});

// button remove value from input search
inputSearch.addEventListener("input", (e: Event | any): void => {
    if (e.target.value == "") {
...
btnAddInput_search.addEventListener("click", (e: MouseEvent | any): void => {
    let searchElement = inputSearch.closest(".search-element") as HTMLElement;
    let searchSection = searchElement.closest('.search-section') as HTMLElement;
    ...
    document.body.addEventListener("click", (e: any) => {
        let blur: Document | bool = e.target.matches(".blur");

        if (blur == false) removeClasses();
    });
...
navBarLists.forEach((list: Element, index: num): void => {
    list.addEventListener("click", (): void => {
        for (let i: num = 0; i < links.length; i++) {
            if (index == i) {
...
let skipL: any, skipR: any;

// keyboard navigate
document.addEventListener("keyup", KeyboardControleVideoPlayer);
function KeyboardControleVideoPlayer(e: KeyboardEvent): void {
    const tagName: str | undefined = document.activeElement?.tagName.toLowerCase();

    switch (e.key.toLowerCase()) {

        // shortcut key  play & pause
        case ' ':
        case 'k': {
        ...
        // shortcut key theater mode
        case 't': {
            theaterModeToggle(null);
            break;
        };

        // shortcut key full screen mode
        case 'f': {
            fullScreenModeToggle();
            break;
        };

        // shortcut key mini player mode
        case 'i': {
            miniPlayerModeToggle();
            break;
        };

        // shortcut key volume control
        case 'm': {
            volumeLevelToggle();
            shortcutKeyVolume();
            break;
        ...
        // shortcut key caption mode (subtitle)
        case 'c': {
...
function palyPaudseVideoToggle(): void {
    if (video.paused) {
        video.play()
        popUp.innerHTML = `
...
const mainContainer = videoPlayer.closest(".main-container") as HTMLElement;

function theaterModeToggle<T>(theaterObj: {
    readonly theaterplayer: str;
    readonly theatermain: str;
} | T): void | null {
...
function fullScreenModeToggle(): void {
...
// toggle mini player mode function
btnMiniPlayer.addEventListener("click", miniPlayerModeToggle);

function miniPlayerModeToggle(): void {
...
// volume button and function
let volumeLevel: str;
btnVolumeLvl.addEventListener("click", volumeLevelToggle);
volumeRange.addEventListener("input", (e: Event | any): void => {
    video.volume = e.target.value;
    video.muted = e.target.value === 0;
    let progressValue: str = e.target.value * 100 + "%";
    lineProgressRange.style.width = progressValue;
});

// disabled controle video for volume up/down
function disabledControleVideo(): void {
    volumeRange.onfocus = () => {
        document.removeEventListener("keyup", KeyboardControleVideoPlayer);
    };
...
function volumeLevelToggle(): void {
...
function volumePercent(percent: str): void {
...
interface VolumeRangeInfo {
    volumeLvl: str,
    readonly volumeVal: str,
    readonly volumeLin: str,
};

video.addEventListener("volumechange", (e) => videoVolumeInfo(e, null));
function videoVolumeInfo(e: event, arrayVolume: VolumeRangeInfo[] | null): void {
    let videoVolume: any = e.target.volume;
    volumeRange.value = video.volume.toString();

    let volumeObj: VolumeRangeInfo = {
...
function shortcutKeyVolume(): void {
    if (video.volume == 0) {
        popUp.innerHTML = `
        <svg viewBox="0 0 36 36" >
            <path fill="#fff"
...
video.addEventListener("loadeddata", (e: event) => {
    totalTime.textContent = formatDuration(e.target.duration);
});

interface TimeProgress {
    readonly lineporcent: str,
    readonly videoct: str,
};

let timeLineProgress: TimeProgress;
...
video.addEventListener("timeupdate", (e: event) => {
    ...
    const percent: num = video.currentTime / video.duration;
...
function formatDuration(time: num): string {
    let seconds: num | str = Math.floor(time % 60);
    let minuts: num = Math.floor(time / 60);
...
// skip time function 
function skipCurrentTime(durationSkip: num): void {
...
const skipPopUpRight = () => {
    ...
    let arrowDirLeft = skiRight.firstElementChild?.children[ 0 ];
    arrowDirLeft?.classList.add("arrowDirLeft");
    videoPlayer.appendChild(skiRight);
    skipR = setTimeout(() => skiRight.remove(), 1000);

    if (video.currentTime == video.duration) skiRight.remove();
};

// skip pop up left
const skipPopUpLeft = () => {
    skipLeft.innerHTML = skipIcon;
    let arrowDirRight = skipLeft.firstElementChild?.children[ 0 ];
    arrowDirRight?.classList.add("arrowDirRight");
    videoPlayer.appendChild(skipLeft);
    skipL = setTimeout(() => skipLeft.remove(), 1000);

    if (video.currentTime == 0) skipLeft.remove();
    if (videoPlayer.classList.contains("rel")) video.play();
};
...
function captionSubTitleModeToggle(): void {
    const isCaption: bool = captionMode.mode === "hidden";
...
interface Liked {
    readonly likeicon: str,
    readonly likenum: str,
};
...
likeBtn.addEventListener("click", (e: event): void => {
    ...
    const liked: bool = !e.target.classList.contains("liked");
...
const dislikeBtn = document.querySelector(".btn-dislike") as HTMLElement;
dislikeBtn.addEventListener("click", (e: event): void => {
...
descriptionToggle.addEventListener("click", (e: event): void => {
    let showMore: event = e.target.matches(".show-more");
    let showMoreChange: str;
...
// time line section
const previewContainer = document.querySelector(".preview-container") as HTMLImageElement;
...
// time line controm target mouse device
// timeLineContainer.addEventListener("mousedown", (e): void => {
//     toggleScrubbing(e);
//     document.addEventListener("mousemove", toggleScrubbing);
// });

// window.addEventListener("mouseup", (): void => {
//     document.removeEventListener("mousemove", toggleScrubbing);
// });

timeLineContainer.addEventListener("mousemove", handleTimeLineUpdate);

// time line controm target Mobile device
timeLineContainer.addEventListener("pointerdown", (e): void => {
    timeLineContainer.setPointerCapture(e.pointerId);
    toggleScrubbing(e);

    timeLineContainer.addEventListener("pointermove", toggleScrubbing);
    timeLineContainer.addEventListener("pointerup", (): void => {
        timeLineContainer.removeEventListener("pointermove", toggleScrubbing);
    }, { once: true });
});

function handleTimeLineUpdate(e: event): void {
    const rect = timeLineContainer.getBoundingClientRect();
    const percent: num = Math.min(Math.max(0, e.x - rect.x), rect.width) / rect.width;
    ...
    let previewImgPercent: num = percent;
    ...
    let timeLineProgressNumber: null | str | num;
...
function getDataFromLocalStorage<T>(arrayVolume: VolumeRangeInfo[] | T,
    theaterObj: {
        readonly theaterplayer: str;
        readonly theatermain: str;
    } | null,
...
let mobileSize: number = 550;

const theaterModeMediaQuery: MediaQueryList = window.matchMedia(`(max-width:${ mobileSize }px)`);
const fullScreenModeMediaQuery: MediaQueryList = window.matchMedia('(max-device-width: 896px) and (orientation: landscape)');
const orientationPortrait: MediaQueryList = window.matchMedia(`(max-device-width: ${ mobileSize }px) and (orientation: portrait)`);

function handleTheatermode(e: event): void {
...
function videoFullscreenLandscape(e: event): void {
...
const videoControlContainer = document.querySelector(".video-controls-container") as HTMLElement;
function mobileTouchVideo(e: event): void {
